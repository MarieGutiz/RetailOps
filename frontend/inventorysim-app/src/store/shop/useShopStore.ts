import {
  runFrontendABC,
  runShopABC,
} from '@/hooks/simulator/modules/abc/frontendABC';
...
import {
  ...
  shopId,
  type ShopSlice,
  type AutogenShopLifecycle,
  type UserShopLifecycle,
  type AutogenShopSlice,
} from '@/types/shop';
...
//This store manages shop metadata, products, inventory, and analytics.
// It also handles the main ABC execution logic and lifecycle management for
//  both user-created and autogenerated shops.
  ...
  persist(
    (set, get) => ({
          ...
          return {
            shops: {
              ...
              [currentShop.id]: {
                ...
                lifecycle:
                  slice.lifecycle === 'CREATED' ? 'IMPORTING' : slice.lifecycle,
              },
            },
          };
        });
      ...
      runABC: async ({
        ...
        const currentShopMeta = explicitShopId ? undefined : get().shop;
        ...
        try {
          ...
          /* ───────────────── BACKEND ABC ───────────────── */
            ...
            if ('items' in output.result) {
              const { products, inventory, analytics } = extractBackendABC(
                output.result as AbcResponseDto
              );
        ...
        shops: Object.fromEntries(
          Object.entries(state.shops).map(([id, shop]) => [
            id,
            {
              ...
              autogenHydrated:
                shop.kind === 'AUTOGEN' ? shop.hydrated : undefined,
              abc: shop.abc,
...
function normalizeSummary(result: SimulatorABCResult): ABCSummary | undefined {
  ...
  if ('categoryA' in result) {
    ...
    return {
      totalValue,
      A: {
        count: result.categoryA.length,
        valuePct: totalValue
          ? (result.categoryA.reduce((sum, p) => sum + (p.unitPrice ?? 0), 0) /
              totalValue) *
            100
          : 0,
      },
      B: {
        count: result.categoryB.length,
        valuePct: totalValue
          ? (result.categoryB.reduce((sum, p) => sum + (p.unitPrice ?? 0), 0) /
              totalValue) *
            100
          : 0,
      },
      C: {
        count: result.categoryC.length,
        valuePct: totalValue
          ? (result.categoryC.reduce((sum, p) => sum + (p.unitPrice ?? 0), 0) /
              totalValue) *
            100
          : 0,
      },
