  ...
  persist(
    (set, get) => ({
      ...
      createShop: ({ name }): UserShopMeta => {
        ...
        // Notify the user
        useNotificationStore.getState().addNotificationRaw({
          id: `shop-created-${id}`,
          msg: `Your shop "${meta.name}" has been successfully created!`,
          type: "success",
          read: false,
          protected: false,
        });
    ...
    ensureAutogenShop: (id: ShopId, label: string) => {
      set((state) => {
        ...
        //Notify about autogen creation
        useNotificationStore.getState().addNotificationRaw({
          id: `autogen-shop-${id}`,
          msg: `Autogenerated shop "${label}" is ready to use.`,
          type: "info",
          read: false,
          protected: true,
        });
