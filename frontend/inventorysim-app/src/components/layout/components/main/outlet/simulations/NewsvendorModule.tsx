...
import NewsvendorForm from '@/views/simulator/newsvendorViews/forms/NewsvendorForm';
import NoSelectShop from '@/views/helpers/NoSelectShop';
import ResultsPanel from '@/views/simulator/newsvendorViews/ResultsPanel/NewsvendorResultsPanel';
...
const NewsvendorModule = () => {
  ...
  const { simulator, lastSimulatedProduct } = useHydratedNewsvendorSimulator(
    selectedShop.id,
    selectedShop.name
  );
      ...
      bg-red-200 backdrop-blur-sm shadow-md 
       ...
       border border-gray-200"
        ...
        <TabsContent value="results" className="mt-8 sm:mt-4 w-full">
          ...
          storeState.newsvendorSimulations[selectedShop.id]?.[
            lastSimulatedProduct
          ] ? (
            <ResultsPanel
              result={
        ...
        <TabsContent value="distribution" className="mt-8 sm:mt-4 w-full">
          {simulator.response && simulator.lastRequest && simulator.pdf ? (
            <DistributionPanel
