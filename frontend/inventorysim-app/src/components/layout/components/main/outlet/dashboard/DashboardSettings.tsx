import {
  ...
  PolicySetting,
  currencyOptions,
} from '@/components/layout/context/Settings';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
 ...
 * 
 * Provides a two-panel layout for simulator configuration:
 * 1. Left panel: Advanced settings in an Accordion
 *    - Currency, Unit, Horizon, Stock Policy
 *    - Reset button to revert to defaults
 * 2. Right panel: Current settings legend
 *    - Displays selected currency (with icon), unit, horizon, and stock policy
 * 
 * Integrates with the simulator store for live updates.
 */

const DashboardSettings = () => {
  const { currency, unit, horizon, stockPolicy, resetSettings } =
    useSimulatorStore();
