              ...
              {[
                ...
                return (
                  <TableHead
                    ...
                    onClick={() =>
                      col.key !== "product" && toggleSort(col.key as typeof sortKey)
                    }
                  >
                    {col.label} {arrow}
                  </TableHead>
          ...
          <TableBody>
            {loading
              ...
              : sortedItems.map(item => {
                  const threshold = item.cumulativePct >= 95 ? "95" : item.cumulativePct >= 80 ? "80" : null
                  const rowClass = [
                    threshold === "95"
                    ? "bg-emerald-100"
                    : threshold === "80"
                    ? "bg-amber-100"
                    : "",
                    hoveredCategory === item.abcCategoryType ? "ring-2 ring-primary/50" : "",
                  ]
                    .filter(Boolean)
                    .join(" ")
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="px-2 py-1 font-medium text-right">{format(item.salesValue)}</TableCell>
                      ...
                      <TableCell className="px-2 py-1 text-right">{item.demandFrequency}</TableCell>
                      <TableCell className="px-2 py-1 text-center">
            ...
            {!loading && (
              <TableRow className="font-semibold bg-background sticky bottom-0">
                ...
                <TableCell className="text-right px-2">{format(summary.totalValue)}</TableCell>
                <TableCell colSpan={3} />
              </TableRow>
            )}
          </TableBody>
