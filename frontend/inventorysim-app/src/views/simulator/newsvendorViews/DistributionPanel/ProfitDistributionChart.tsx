...
const ProfitDistributionChart = ({
  ...
  const [probabilityOfLoss, setProbabilityOfLoss] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const totalRuns = Object.values(result.histogram).reduce(
          (a, b) => a + b,
          0
        );
    ...
    <div className="w-full space-y-4">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mt-1">
          Distribution of simulated profits from {request.simulationRuns} Monte
          Carlo runs.
        </p>
      </div>
        ...
        <>
          <div className="w-full">
            <ResponsiveContainer width="100%" aspect={1.8}>
              ...
              >
                ...
                <ReferenceLine x={0} stroke="red" strokeDasharray="4 4" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm text-gray-700">
            ...
            {minProfit !== null && maxProfit !== null && (
              <div>
                <strong>Profit Range:</strong> {minProfit.toFixed(2)} to{' '}
                {maxProfit.toFixed(2)}
              </div>
            )}
          ...
          <div className="text-[11px] sm:text-xs text-muted-foreground">
            Demand sampled from Normal(μ={request.meanDemand}, σ=
            {request.stdDeviation}) across {request.simulationRuns} simulations.
          </div>
        </>
      )}
