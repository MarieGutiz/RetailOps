...
const ProductTable = ({
  ...
  const { sorting, setSorting, search, setSearch, pagination, setPagination } =
    useProductTableState();

  //For inventory status
  const { isProductInInventory, removeFromInventory, inventory } =
    useInventoryStore();
  //  const inventoryReady = useInventoryStore((s) => !!s.shopMeta);

  //Confirm deleting from inventory
  ...
  const columns = useMemo<ColumnDef<Product>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <RowActions product={row.original} readonly={readonly} />
        ),
      ...
      <div className="rounded-lg border overflow-hidden">
        <div className="relative max-h-[520px] overflow-y-auto overflow-x-auto">
          <div className="inline-block min-w-full align-top">
            ...
            {/* Add to inventory is product as well as shop exists */}
            {selectedProduct && shopMeta && (
              <AddToInventoryDialog
                ...
                inventoryItem={inventory.find(
                  (item) => item.productId === String(selectedProduct.id)
                )}
            ...
            {productToRemove && shopMeta && (
              <ConfirmActionDialog
                ...
                description={
                  <>
                    Remove <b>{productToRemove.name}</b> from the inventory of{' '}
                    <b>{shopMeta.name}</b>?
                    <br />
                    <span className="text-xs text-muted-foreground">
                      This will remove the product and its quantity from the
                      current simulation.
                    </span>
                  </>
                }
                ...
                variant="danger"
                onConfirm={() => {
                  removeFromInventory(String(productToRemove.id));
