  ...
  const columns = useMemo<ColumnDef<InventoryRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <SortableHeader column={column} label="Unit Cost" />
        ),
        cell: ({ row }) => (
          <span className="tabular-nums">
            {currency}
            {row.original.product.unitCost.toFixed(2)}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="tabular-nums">
            {currency}
            {row.original.product.unitPrice.toFixed(2)}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="tabular-nums font-medium">
            {currency}
            {row.original.inventoryValue.toFixed(2)}
          </span>
        ),
      },
      {
        ...
        <TableBody>
          {loading ? (
            ...
            table.getRowModel().rows.map((row) => {
              const abcClass = row.original.abcClass as
                | 'A'
                | 'B'
                | 'C'
                | undefined;
