...
const RowActions = ({
  ...
  /* ---------------- permissions ---------------- */
  const canEditProduct = !readonly;
  const canDeleteProduct = !readonly;
  const canModifyInventory = true; // inventory is allowed even for autogen shops
  ...
  const [confirm, setConfirm] = useState<
    null | 'delete-product' | 'remove-inventory'
  >(null);
  ...
  const inventoryItem = useInventoryStore((state) =>
    productId
      ? state.inventory.find((item) => item.productId === productId)
      : undefined
  );
          ...
          <div className="flex justify-center">
            <DropdownMenu modal={false}>
              ...
              <DropdownMenuContent align="end" className="min-w-[190px]">
                ...
                {/* DELETE */}
                <DropdownMenuItem
                  className="flex items-center gap-2 text-red-600 data-[disabled]:opacity-50"
                  disabled={!canDeleteProduct}
                  onSelect={() => {
                    if (!canDeleteProduct) return;
                    setConfirm('delete-product');
                  }}
                >
                ...
                >
                  {isInInventory ? (
                    <>
                      <Pencil className="h-4 w-4 opacity-70" />
                      <span>In inventory (Qty: {inventoryItem?.quantity})</span>
                    </>
                  ) : (
                    <>
      ...
      <ConfirmActionDialog
        ...
        description={
          confirm === 'delete-product' ? (
            ...
            <>
              Are you sure you want to remove <b>{product.name}</b> from
              inventory?
              <br />
