import { isAbcLog, type SimulationLogEntry } from "@/views/Overview/hooks/useSimulationBitacora";
...
import { computeAnalyticsForLog, computeDelta } from "./hooks/computeAnalyticsForLog";
       ...
       className="
        ...
        <TabsTrigger
         ...
         className={`
            px-2 sm:px-4 py-1 sm:py-2
            rounded-md
            transition-colors ${activeTab === "breakdown" ?
             "jbtn-success shadow-inner" : "bg-blue-100 hover:bg-blue-200"}`}
      ...
      <TabsContent value="whatif" className="mt-6 space-y-6 max-w-md">
        <div>
          ...
          <Input
            ...
            onChange={e =>
              setThresholdInput(prev => ({ ...prev, a: Number(e.target.value) }))
            }
          />
        </div>
        ...
        <div>
          ...
          <Input
            ...
            onChange={e =>
              setThresholdInput(prev => ({ ...prev, b: Number(e.target.value) }))
            }
          />
        </div>
        ...
        {validatedThresholds && (
          <div className="text-sm text-muted-foreground">
            A ≤ {validatedThresholds.a}% <br />
            B ≤ {validatedThresholds.b}% <br />
            C &gt; {validatedThresholds.b}%
          </div>
        )}
          ...
          <>
            ...
            {powerLawDiagnostics && (
              <div className="p-3 bg-indigo-50 rounded text-sm text-indigo-800">
                ...
                <br />
                • Top {powerLawDiagnostics.pctItemsFor80.toFixed(1)}% of items
                generate 80% of total value.
                <br />
                • Top {powerLawDiagnostics.pctItemsFor95.toFixed(1)}% of items
                generate 95% of total value.
              </div>
            )}
