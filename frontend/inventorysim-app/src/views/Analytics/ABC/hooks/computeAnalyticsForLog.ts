...
--------------------------------*/
export const computeClassicABC = (data: any[], thresholds: { a: number; b: number; c: number }) => {
  return ABCAnalysisFrontend(data, thresholds);
};

const computeMultiABC = (data: any[], thresholds: { a: number; b: number; c: number }, valueWeight = 0.7, demandWeight = 0.3) => {
  if (!data.length) {
    return {
      table: [],
      summary: {
...
export const computeDelta = (
  ...
  const newMap = new Map(newAnalytics.table.map(i => [i.product.id, i.category]));
